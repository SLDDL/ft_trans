class PongGame {
    ...
    createScene() {
        ...
        const camera = new BABYLON.UniversalCamera('camera', new BABYLON.Vector3(0, 15, 0), this.scene);
    ...
    setupControls() {
        ...
        window.addEventListener('keydown', (event) => {
            ...
            if (event.code === 'Space' && !this.inMenu) {
    ...
    updatePaddles(deltaTime) {
        ...
        if (this.keys['ArrowUp'] && this.paddle1.position.z > -this.gameHeight / 2 + this.paddleHeight / 2) {
            this.paddle1.position.z -= paddleMovement; // Arrow Up moves up (negative Z)
        }
        if (this.keys['ArrowDown'] && this.paddle1.position.z < this.gameHeight / 2 - this.paddleHeight / 2) {
            this.paddle1.position.z += paddleMovement; // Arrow Down moves down (positive Z)
        }
        
        // Player 2 controls or AI
        if (this.gameMode === 'human-vs-human') {
            // Human Player 2 controls (W/S keys)
            if (this.keys['KeyW'] && this.paddle2.position.z > -this.gameHeight / 2 + this.paddleHeight / 2) {
                this.paddle2.position.z -= paddleMovement; // W moves up (negative Z)
            }
            if (this.keys['KeyS'] && this.paddle2.position.z < this.gameHeight / 2 - this.paddleHeight / 2) {
                this.paddle2.position.z += paddleMovement; // S moves down (positive Z)
            }
    ...
    resetBall() {
        ...
        if (this.gameStarted) {
            this.gameRunning = false;
            setTimeout(() => {
                if (this.gameStarted && !this.inMenu) {
